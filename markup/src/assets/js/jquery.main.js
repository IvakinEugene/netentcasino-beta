...
function initSameHeightRows() {
	...
	// maxItemsNumber = Math.max.apply(null, items);
	...
	function calculate() {
		// i - количество элементов в блоке
		//  j - количетво самих блоков
		for (var i = 0; i < 3; i++) {
			...
			for (var j = 0; j < boxes.length; j++) {
				// if (jQuery(items[j][i]).outerHeight() > maxHeight) {
					jQuery(items[j][i]).css('min-height', maxHeight);
				// }
			}
			maxHeight = 0;
...
// init slickCarousel
function initSlick() {
	var slidersHolder = jQuery('.games-slider-holder');
	...
	if (slidersHolder) {
		if (slidersHolder.find('.games-slider').length > 1) {
			$('.games-slider').slick('unslick');
			$('.games-slider').slick({
				slidesToShow: 2,
				slidesToScroll: 2,
				arrows: false,
...
function initOpenClose() {
	ResponsiveHelper.addRange({
		'..1024': {
			...
			off: function() {
				if (jQuery('.open-close').data('OpenClose')) {
		...
		'..768': {
			...
			off: function() {
				if (jQuery('.dropdown-nav').data('OpenClose')) {
					jQuery('.dropdown-nav').openClose('destroy');
				}
			}
		}
	});
...
function initStickyScrollBlock() {
	...
	jQuery('.content-navigation').stickyScrollBlock({
		setBoxHeight: true,
		activeClass: 'fixed-position',
...
function initAnchors() {
	new SmoothScroll({
		...
		extraOffset: function() {
			...
			if ((jQuery(window).width() < 1024) && (jQuery('.fixed-aside').length)) {
				console.log(2)
			...
			totalHeight += jQuery('#header').outerHeight() + jQuery('#wpadminbar').outerHeight() - 1;
...
;(function() {
	...
	var getWindowHeight = function() {
		var value = window.innerHeight || document.documentElement.clientHeight;

		if (jQuery('#wpadminbar').length) {
			value -= jQuery('#wpadminbar').outerHeight();
		}
  ...
  $.fn.fitVids = function( options ) {
    ...
    return this.each(function(){
      ...
      $allVideos.each(function(){
        ...
        $this.wrap('<div class="fluid-width-video-holder"><div class="fluid-width-video-wrapper"></div></div>').parent('.fluid-width-video-wrapper').css('padding-top', (aspectRatio * 100)+'%');
...
(function( $ ) {
	...
	$.widget( "app.autocomplete", $.ui.autocomplete, {
		...
		_renderItem: function( ul, item ) {
			...
			$( "<a/>" ).attr( "href", "#" )
					   .html("<img src=" + item.icon + "><span class=\"title\">" + label + " <span class=\"category\">(" + item.category + ")</span></span>")
					   .appendTo( $li );
			return $li;
		}
	});
	...
	var tags = [
		{
			label: "Betsson",
			category: "Casino",
			icon: "http://via.placeholder.com/43/6daf0e/ffffff"
		},
		{
			label: "Bet365",
			category: "Casino",
		...
		{
			label: "Blackjack Betting strategies",
			category: "Guide",
			icon: "http://via.placeholder.com/43/6daf0e/ffffff"
		},
		{
			label: "Slots Betting tips",
			category: "Guide",
			icon: "http://via.placeholder.com/43/6daf0e/ffffff"
		},
		{
	...
	$(function() {
		$( "#m-autocomplete" ).autocomplete({
			appendTo: ".mobile-nav-holder .top > div",
			source: tags,
			open: function(event, ui) {
				$(this).closest('.mobile-nav-holder').addClass('ui-drop-active');
				$(this).closest('.mobile-nav-holder').addClass('changed');
			},
			close: function(event, ui) {
				$(this).closest('.mobile-nav-holder').removeClass('ui-drop-active');
			}
		});
		$( "#d-autocomplete" ).autocomplete({
			appendTo: "#search-popup .popup-frame",
			source: tags,
			open: function(event, ui) {
				$(this).closest('.default-popup').addClass('ui-drop-active');
				$(this).closest('.default-popup').addClass('changed');
			},
			close: function(event, ui) {
				$(this).closest('.default-popup').removeClass('ui-drop-active');
				if ($(this).val().length === 0) {
					$(this).closest('.default-popup').removeClass('changed');
				}
			}
		});
	});
